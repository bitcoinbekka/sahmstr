...
interface AITagButtonProps {
  /** Kick off the paid tagging request. */
  onTag: () => void;
  /** Current flow stage from useContextVMVision. */
  stage: 'idle' | 'sending' | 'awaiting-payment' | 'paying' | 'thinking' | 'done' | 'error';
  /** Whether a provider is configured. */
  isConfigured: boolean;
  /** Whether the user has a Lightning wallet connected. */
  hasWallet: boolean;
    ...
    return (
      <div className="rounded-lg border-2 border-dashed bg-muted/30 p-3 text-center">
        <p className="text-xs leading-relaxed text-muted-foreground">
          AI photo tagging is coming soon — you'll be able to pay a few sats to have
          the details read from your photo automatically.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      <Button
        ...
        onClick={onTag}
        disabled={disabled || busy || !hasWallet}
      >
        {busy ? (
